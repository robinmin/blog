(() => {
	...
	function me(n, e, t, r) {
		const { fragment: s, after_update: l } = n.$$;
		s && s.m(e, t),
			r ||
				Rt(() => {
					const i = n.$$.on_mount.map(bt).filter(at);
	...
	var _t = class {
		constructor(e) {
			...
			delete e.element,
				...
				delete e.autofocus,
				delete e.sort;
			const T = t instanceof HTMLElement ? t : document.querySelector(t);
			T
				? (this._pfs = new Tr({
						target: T,
						props: {
		...
		triggerFilters(e) {
			...
			for (const [r, s] of Object.entries(e))
				if (Array.isArray(s)) for (const l of s) t[`${r}:${l}`] = !0;
				else t[`${r}:${s}`] = !0;
